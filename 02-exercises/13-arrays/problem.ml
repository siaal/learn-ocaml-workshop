...
let double array : unit = Array.iteri array ~f:(fun i x -> Array.set array i (x * 2))
...
let double_selectively array indices : unit =
  List.iter indices ~f:(fun idx -> Array.set array idx (array.(idx) * 2))
;;
...
let double_matrix (matrix : int array array) : unit = Array.iter matrix ~f:double
